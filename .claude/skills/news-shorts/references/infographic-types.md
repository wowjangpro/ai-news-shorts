...
```python
{
    "type": "stat",
    "title": "석유제품 가격 상승",          # 제목
    "value": "두바이유 +10.4%",            # 대표 수치 (크게 표시)
    "sub_items": [
        {"label": "두바이유", "value": "+10.4%"},
        {"label": "경유", "value": "+7%"},
    ],
    "accent_color": [255, 150, 50]         # 선택
}
```

상단에 제목+대표 수치, 하단에 세부 항목. sub_items 2~4개 권장.
...
```python
{
    ...
    "title": "경선 일정",                   # 선택
    "events": [
        {"date": "3/24~4/10", "text": "토론회 2차례"},
        {"date": "4/18", "text": "최종 후보 발표"},
    ],
    "accent_color": [230, 60, 80]          # 선택
}
```

날짜와 이벤트를 시각적으로 분리 표시. events 3~5개 권장.
...
**위 7개 타입만 사용 가능.** 이 외의 타입을 사용하면 인포그래픽이 생성되지 않음.
